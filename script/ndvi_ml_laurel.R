...
pred_h2o <- h2o.predict(deep_2, newdata = test_h2o)
...
fig <- plot_ly(data_ndvi_ml, x = data_ndvi_ml$Fecha, y = data_ndvi_ml$Laurel, name = colnames(data_ndvi_ml)[2], type = 'scatter', mode = 'lines') 
fig <- fig %>% add_trace(y = data_ndvi_ml$Pred, name = colnames(data_ndvi_ml)[3], mode = 'lines+markers',connectgaps = TRUE) 
fig_la <- fig %>% layout(title = "Modelo DeepLearning NDVI Laurel",
